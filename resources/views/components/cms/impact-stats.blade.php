...
<div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
    <div class="p-8 bg-white rounded-lg shadow-sm">
        ...
        <div class="font-bold text-4xl mb-1 text-flame-red counter" data-count="{{ trim(str_replace(['+', ','], '', $volunteers ?? '500')) }}">
            {{ $volunteers ?? '500+' }}
        </div>
        <p class="text-gray-600">{{ $volunteersLabel ?? 'Dedicated Volunteers' }}</p>
    </div>
    ...
    <div class="p-8 bg-white rounded-lg shadow-sm">
        ...
        <div class="font-bold text-4xl mb-1 text-flame-red counter" data-count="{{ trim(str_replace(['+', ','], '', $hours ?? '10000')) }}">
            {{ $hours ?? '10,000+' }}
        </div>
        <p class="text-gray-600">{{ $hoursLabel ?? 'Hours Contributed' }}</p>
    </div>
    ...
    <div class="p-8 bg-white rounded-lg shadow-sm">
        ...
        <div class="font-bold text-4xl mb-1 text-flame-red counter" data-count="{{ trim(str_replace(['+', ','], '', $events ?? '20')) }}">
            {{ $events ?? '20+' }}
        </div>
        <p class="text-gray-600">{{ $eventsLabel ?? 'Community Events Held' }}</p>
    </div>
</div>
